...
8. **旧版本账本安全升级（v1 → v2）**：打开无链头锚点的旧账本时不直接判损坏，
   而是先证明再迁移，全程在单个事务内完成：
   - **交叉证据**：`voucher.post` 审计集合必须等于凭证表、`period.close` 的
     snapshot_id 集合必须等于快照表、重估/反结账审计引用的凭证与快照必须真实存在；
   - **哈希链连续**：逐条重算旧审计链与快照链的哈希与 prev 链接；
   - 两者都通过才补写锚点、置 `schema_version=2` 并留痕一条 `schema.migrate`
     审计（幂等：重复打开不会重复迁移）；空旧账本只置版本号，不伪造锚点或迁移记录。
   - 任一项无法证明（末条审计被抹、快照缺失、记录内容被改等）则**保持阻断、
     不写入任何锚点**，账本原样保留待人工处理。迁移完成后的账本与新账本完全同构，
     末条记录删除依旧被锚点拦截。

## 验证结果

`python3 verify.py` 在临时账本上实跑全部业务场景，最近一次运行 **106/106 通过**，覆盖：
...
- **旧版本账本升级**：完整无锚点旧链（含已关账期）打开后自动迁移，随后走完核对、
  关账阻断、反结账、追溯调整、2 月过账/重估/再关账全流程，迁移幂等；
  末条关账审计被删、末张凭证缺过账审计、旧记录内容被改、快照缺失四类不可证明
  连续性的旧账本全部打开即阻断且锚点不落库；空旧账本仅置版本号；
  全新空账本跨进程重开不被误判；迁移后删末条审计仍被拦截；
