...
8. **旧版本账本安全升级（v1 → v2）**：打开无链头锚点的旧账本时不直接判损坏，
   ...
   - **链尾证据分级**：若现存末条审计是凭证过账/关账（业务表能逐条验证到它），
     交叉证据已足够；若旧库**没有任何凭证或快照**（纯主数据旧链，末条停在
     币种/科目/账户/汇率/配置/反结账等事件上），内部表无法证明"这就是最后一条"，
     必须额外出示**链尾清单**（默认自动查找 `<账本>.tail.json`，
     CLI 可用 `--tail-manifest <文件>` 指定、`--tail-manifest none` 禁用）：
     ```json
     {"manifest_version": "1",
      "audit_count": 12, "audit_tail_hash": "<末条审计 SHA-256>",
      "snapshot_count": 0, "snapshot_tail_hash": null}
     ```
     条数或末条哈希与现存账本不符（覆盖"缺尾但链被重接"的情形）、文件缺失或
     损坏，一律**保持阻断、不写入任何锚点**；
   - 证据齐全才补写锚点、置 `schema_version=2` 并留痕一条幂等的 `schema.migrate`
     审计（记录采用的证据类型）；空旧账本只置版本号，不伪造锚点或迁移记录。
   - 迁移完成后的账本与新账本完全同构，末条记录删除依旧被锚点拦截。

## 验证结果

`python3 verify.py` 在临时账本上实跑全部业务场景，最近一次运行 **121/121 通过**，覆盖：
...
- **纯主数据旧链（无凭证/快照可交叉验证）**：完整链在无证据/禁用证据时阻断，
  提供与链尾条数、哈希一致的链尾清单后迁移成功，并继续走完过账/重估/关账；
  缺尾（删除末条并重接 prev 链使哈希链仍连续）凭外部证据发现并阻断、锚点不落库；
  陈旧证据（条数/哈希不符）、证据文件损坏同样阻断；以凭证/关账收尾的旧库有充分
  内部证据，不依赖外部清单仍正常迁移；
